...
fn test_withdraw_success() {
    ...
    let share_price: u64 = 1_000_000; // 1:1 ratio
    let shares_to_burn: u64 = 3_000_000; // 3 shares
    // base_to_return = 3_000_000 * 1_000_000 / 10^6 = 3_000_000
    ...
    let accounts = vec![
        ...
        (vault_base_ata, create_token_account(&base_mint, &vault_key, 10_000_000)), // vault has 10 USDC
    ...
    assert_eq!(base_received, 3_000_000);
    ...
    assert_eq!(vault_remaining, 10_000_000 - 3_000_000);
}
...
#[test]
fn test_withdraw_missing_signer() {
    let mollusk = setup_with_token();
    let admin = Pubkey::new_unique();
    let withdrawer = Pubkey::new_unique();
    let base_mint = Pubkey::new_unique();
...
#[test]
fn test_withdraw_wrong_share_mint() {
    let mollusk = setup_with_token();
    let admin = Pubkey::new_unique();
    let withdrawer = Pubkey::new_unique();
    let base_mint = Pubkey::new_unique();
    ...
    let accounts = vec![
        (withdrawer, Account::new(1_000_000_000, 0, &Pubkey::default())),
        (withdrawer_share_ata, create_token_account(&wrong_mint, &withdrawer, 1_000_000)),
        (wrong_mint, create_mint(&vault_key, 6, 1_000_000)),
        (vault_base_ata, create_token_account(&base_mint, &vault_key, 10_000_000)),
        (withdrawer_base_ata, create_token_account(&base_mint, &withdrawer, 0)),
        (vault_key, make_vault_account(vault_data)),
        mollusk_svm_programs_token::token::keyed_account(),
