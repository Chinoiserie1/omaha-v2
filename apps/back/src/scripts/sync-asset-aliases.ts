...
const STOCK_TICKERS: Record<string, StockEntry> = {
  ...
  // xStock wins — tradeable on Jupiter swap/v1 (11):
  ACN:   { name: "Accenture",           bestSuffix: "x",  hasXstock: true,  hasOndo: true },
  AVGO:  { name: "Broadcom",            bestSuffix: "x",  hasXstock: true,  hasOndo: true },
  GME:   { name: "GameStop",            bestSuffix: "x",  hasXstock: true,  hasOndo: true },
  JNJ:   { name: "Johnson & Johnson",   bestSuffix: "x",  hasXstock: true,  hasOndo: true },
  JPM:   { name: "JPMorgan",            bestSuffix: "x",  hasXstock: true,  hasOndo: true },
  MA:    { name: "Mastercard",          bestSuffix: "x",  hasXstock: true,  hasOndo: true },
  MSFT:  { name: "Microsoft",           bestSuffix: "x",  hasXstock: true,  hasOndo: true },
  NFLX:  { name: "Netflix",             bestSuffix: "x",  hasXstock: true,  hasOndo: true },
  TQQQ:  { name: "ProShares UltraPro QQQ", bestSuffix: "x", hasXstock: true, hasOndo: true },
  V:     { name: "Visa",                bestSuffix: "x",  hasXstock: true,  hasOndo: true },
  XOM:   { name: "Exxon Mobil",         bestSuffix: "x",  hasXstock: true,  hasOndo: true },
  // Ondo wins — kept as "on" (Ondo routes via JupiterZ RFQ only, GLAM doesn't support yet) (15):
