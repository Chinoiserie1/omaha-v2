...
export async function syncKolProfile(kolId: string): Promise<void> {
  ...
  await kolRepo.updateKolProfile(kolId, {
    restId: user.rest_id,
    ...(user.legacy.name !== undefined ? { displayName: user.legacy.name } : {}),
    ...(user.legacy.followers_count !== undefined ? { followersCount: user.legacy.followers_count } : {}),
    ...(user.legacy.profile_image_url_https !== undefined ? { avatarUrl: user.legacy.profile_image_url_https } : {}),
    ...(user.legacy.description !== undefined ? { bio: user.legacy.description } : {}),
  });
...
export async function syncKolTweets(kolId: string): Promise<number> {
  ...
  logger.info(
    { username: freshKol.username, upserted: newCount, threadTweets: threadCount },
    "Tweets synced"
  );
  return newCount;
}
...
export async function addTweetByUrl(url: string): Promise<{ kol: Kol; tweet: Tweet }> {
