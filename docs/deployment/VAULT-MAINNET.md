...
| **Env Var** | `KEEPER_PRIVATE_KEY` (stays in `.env` permanently) |
...
KEEPER_PRIVATE_KEY='<base58 from admin-program-mainnet-keypair.json>'
...
- Admin keypair (`KEEPER_PRIVATE_KEY`) is the only key on the backend server
